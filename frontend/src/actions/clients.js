import axios from "axios";

const fetchBestClients = () => {
  return [
    {
      rank: 6,
      id: 1,
      name: "Phyllis P. Fry",
      email: "PhyllisPFry@rhyta.com",
      total_spent: 48367,
    },
    {
      rank: 7,
      id: 2,
      name: "Brian P. Gossett",
      email: "BrianPGossett@dayrep.com",
      total_spent: 45776,
    },
    {
      rank: 2,
      id: 3,
      name: "Mike S. Garcia",
      email: "MikeSGarcia@jourrapide.com",
      total_spent: 71441,
    },
    {
      rank: 8,
      id: 4,
      name: "Richard A. Burton",
      email: "RichardABurton@rhyta.com",
      total_spent: 43161,
    },
    {
      rank: 10,
      id: 5,
      name: "Eric A. Tedrow",
      email: "EricATedrow@rhyta.com",
      total_spent: 14072,
    },
    {
      rank: 1,
      id: 6,
      name: "Mary G. Ford",
      email: "MaryGFord@dayrep.com",
      total_spent: 74957,
    },
    {
      rank: 5,
      id: 7,
      name: "Zelda D. Hodges",
      email: "ZeldaDHodges@dayrep.com",
      total_spent: 57823,
    },
    {
      rank: 4,
      id: 8,
      name: "Sheila T. Britt",
      email: "SheilaTBritt@rhyta.com",
      total_spent: 66871,
    },
    {
      rank: 3,
      id: 9,
      name: "John S. Perez",
      email: "JohnSPerez@teleworm.us",
      total_spent: 68579,
    },
    {
      rank: 9,
      id: 10,
      name: "Chris S. Navarro",
      email: "ChrisSNavarro@dayrep.com",
      total_spent: 24669,
    },
  ];
};

const fetchUnitsSold = (id, year, monthly) => {
  return axios.get(`/api/customer/${id}/sales/${year}?monthly=${monthly}`);
};

const fetchUnitsOrdered = (id, year, monthly) => {
  return axios.get(
    `/api/customer/${id}/sales-orders/${year}?monthly=${monthly}`
  );
};

export { fetchBestClients, fetchUnitsSold, fetchUnitsOrdered };
